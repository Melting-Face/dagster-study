...
- **`security`·`archivist`·`skill-matcher`·`tech-writer` 4종은 director 관할 밖**이다 — 배정 주체는 supervisor다.
  ...
  🔴 **관할 밖의 근거가 둘로 갈린다**(2026-08-20) — 앞 3종은 **계층 자체를 감사·기록**하기 때문이고,
  `tech-writer`는 **director의 행동 규칙이 담긴 정본**(이 문서)을 쓰기 때문이다. 도메인이 아니라
  **이해충돌**이 기준이다. 관할 밖이 4종이 되면서 블랙리스트(`disallowedTools`)와 화이트리스트의
...
| `hooks` | 아니오 | 🟢 `analyst`·`tech-writer` **실발동 확인** | **워커별 경로 강제의 유일한 수단**이고 **작동한다**(2026-08-20 각각 3셀 대조 — 가드 `permissionDecisionReason` 원문·즉시 `deny`). 과거 `analyst` 미발동의 원인도 규명됐다: **hooks는 정의 로드 시점에 스냅샷**되어 세션 도중 추가한 배선은 반영되지 않는다 → **hooks를 고치면 새 세션에서 재검증**한다. 🔴 확인된 것은 **`Write`·`Edit`·`NotebookEdit` 경로뿐**(`Bash` 경유는 matcher 밖 = 규율). `researcher`는 `Write` 자체가 없어 **가드에 도달하지 않는다**(미확인) |
...
| 4 | 에이전트 정의 내 `hooks` | **그 워커만** | ✅ **결정적 강제**(2026-08-20 실발동 확인) | `Bash` 경유 쓰기는 matcher 밖 → 5층이 받는다. 🔴 **정의 로드 시점 스냅샷**이라 세션 도중 편집은 무효 — 고치면 **새 세션에서 3셀 대조** |
...
- **2026-08-20 개정**: `tech-writer` `allow`를 **`docs/` · `README.md`** 로 넓히고 `director` `allow: ()`를 추가했다.
  🔴 **둘 다 실발동 `미확인`이다** — 넓힌 뒤의 대조를 아직 돌리지 않았고, 가드 **스크립트 본문** 변경이
  `hooks` 스냅샷 법칙의 적용을 받는지도 미측정이다(배선은 그대로라 즉시 반영일 가능성이 높으나 **확인 전엔 모른다**).
...
| **security**<br/>(계층 밖) | `Agent` 툴 워커(읽기 전용) | **director 결정의 최종 컨펌** — **계획(G1)·작업내용(G2)·계획 델타(Δ)** 를 판정해 `[승인]`/`[반려]`. 노출·규제·거버넌스 점검 | 직접 수정·실행 · director의 지휘를 받는 것 |
