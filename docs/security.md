...
대용량 테이블 `IcebergTableResource` 바인딩을 단일 출처로 재사용한다. **안전 순서
`expire snapshots → remove orphan files`** 를 op 의존성(`dg.In(dg.Nothing)`)으로 강제한다.

| 단계 | 실행 | 근거 |
| --- | --- | --- |
| 1. 스냅샷 만료 | pyiceberg 0.11.x `table.maintenance.expire_snapshots().older_than(dt).commit()` (`older_than`는 tz-aware datetime) | 보존기간 `SNAPSHOT_RETENTION_DAYS`(기본 7일) 경과분 |
| 2. orphan 정리 | **Trino** `ALTER TABLE iceberg.<ns>.<table> EXECUTE remove_orphan_files` | pyiceberg 0.11.x 미지원 → Trino 프로시저. retention 생략 시 기본 7일(min-retention) |
...
> **보완(컴팩션)**: 대용량 append 테이블은 작은 파일이 쌓이므로, 만료 앞단에 **컴팩션**
> (Trino `ALTER TABLE ... EXECUTE optimize`)을 두는 것이 좋다(권장 순서 **compact → expire → orphan**).
