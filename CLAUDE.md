...
- **에이전트 오케스트레이션·기록관**: AI 세션을 **3계층(supervisor→director→subagent)** 으로 나눈다(**director는 우선 1명**,
  ...
  **hook 결정이 실제로 집행됨**을 실증했다.
  ...
  명령은 사람에게 안 올라온다. **확실히 멈춰야 하는 경계는 `deny`로 둔다.**
  🔴 **그래서 `ask` 규칙은 "일부러 위반시켜 확인"이 원리상 불가능하다**(2026-08-21 6셀 실측) —
  프로브가 **위험해야** 프롬프트가 뜨는데, 위험하면 실행할 수 없다. 안전하려고 `--help`를 고르면
  **그 안전함이 바로 분류기가 삼키는 조건**이 된다. 실제로 신규 `ask`가 통과했고, **기존 `ask`
  (`Bash(*git*commit*)`)로 대조군을 돌리자 그것도 통과**해 "내 규칙이 틀렸다"가 기각됐다.
  **`ask`의 실효는 임시 `deny` 전환으로만 간접 확인**된다(전환 시 차단 3/3·과차단 0).
  ⇒ `ask` 규칙이 있다는 사실을 **"반드시 멈춘다"로 읽지 않는다** — 멈추는 것은 **규칙 × 분류기 판정**의 곱이다.
  비가역 명령은 **`ask` + 워커 지시문 금지 조항의 이중 방어**로 두고, 어느 쪽도 단독으로는 봉쇄가 아니다.
  🔴 **단 흡수 여부는 도구 축에 따라 갈린다**(2026-08-21 보완 실측 — `Bash` 축은 이 저장소 직접 관측,
  파일 도구 축은 병렬 세션이 사용자 화면으로 확인): **`Bash`는 실제 위험 호출이면 발동한다**
  (`git push origin main` ✅ — 흡수된 것은 `--help` 류 무해 호출뿐이라 `ask`는 죽은 규칙이 아니다).
  반면 **파일 도구(`Edit`/`Write`/`NotebookEdit`)는 경로 민감도와 무관하게 흡수**됐다 —
  `docs/conventions/**`·`.claude/agents/**`·**`.env.*`** 쓰기가 **3/3 안 떴다**.
  ⇒ **급소는 "경로가 민감한가"가 아니라 "어느 도구인가"이고, 파일 경로 경계는 `deny`여야 확실히 막힌다.**
  ⚠️ **위 §스킬 항목의 "4종 전부 확인 프롬프트 발동을 확인했다"는 auto 모드가 아닌 조건의 관측**이다 —
  조건을 빼면 "항상 막힌다"로 읽힌다. `escalate`·`Write(<경로>)` 죽은 규칙과 같은 계열이고,
  **이번이 더 조용하다(에러 배너조차 없다).**
  🔴 **`permissions`와 `hooks`는 반영 시점이 반대다** — 위 실험에서 임시 `deny`가 **같은 세션에서
  즉시 발동**했다(`permissions`는 세션 도중 반영). 반면 **`hooks`는 정의 로드 시점 스냅샷**이라
  새 세션이 필요하다. **둘을 "설정 파일"로 묶어 같은 규칙으로 기억하면 틀린다.**
  🔴 **선언 목록과 구현 목록은 주기적으로 대조한다** — 이 문서가 *"`ask`로 못 박는다"* 고 명시한
  비가역 작업 중 **`kubectl apply`·`terraform apply`·`terraform destroy`에 규칙이 아예 없었다**
  (2026-08-21 감사 발견·추가). `git push`·`DROP`·`.env`는 있었으므로 **한 번도 대조한 적이 없었던 것**이다. 또 **가드가 막은 것과
