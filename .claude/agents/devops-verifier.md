...
| 상황 | 스킬 | 비고 |
| --- | --- | --- |
| 컨테이너 상태·로그 해석, OOM·기동 실패 원인 | `docker-expert` | ⚙️ 런타임. **진단 목적만** — 스킬이 제안하는 수정·재기동은 실행하지 않는다 |
| 파드 크래시·리소스 한도·이벤트 해석 | `kubernetes-specialist` | ⚙️ 동일 — `describe`/`logs` 해석까지만 |
...
- **스킬이 수정·재기동을 권해도 실행하지 않는다.** 두 스킬 모두 "고치는" 절차를 포함하지만, 이 워커는 판정자다 —
