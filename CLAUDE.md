...
- **에이전트 오케스트레이션·기록관**: AI 세션을 **3계층(supervisor→director→subagent)** 으로 나눈다(**director는 우선 1명**,
  ...
  **같은 논리로 "공개"도 새 축이 아니라 새 도메인**이라 구현 축 **`tech-writer` 1명**만 둔다(2026-08-20 신설,
  판정은 `security`·`data-verifier` 재사용). 쓰기는 **`docs/posts/**` 한정**이고 `docs/analyses/**`(내부 결론)는
  `analyst` 소관이다 — **독자가 다르다**(아는 사람 ↔ 모르는 사람). 🔴 **매체는 축이 아니다** — "티스토리용"·
  ...
  *구조의 사실*이 아니다(원칙 7). 신설 후 호출이 실패하면 **포기하지 말고 잠시 뒤 재시도**한다.
  ✅ **그 덕에 `worker_path_guard.py`의 실발동을 이 저장소에서 처음 확인**했다 —
  `tech-writer` 3셀 대조(위반 2 차단·대조군 통과), 차단 문구는 **가드 원문**이고 즉시 `deny`였다.
  🔴 단 `analyst`의 과거 미발동은 **미해명**이고, 확인된 건 **`Write`/`Edit`/`NotebookEdit` 경로뿐**이다.
