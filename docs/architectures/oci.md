...
- **채택 방향**: 로컬 **kind on Podman**(**8 CPU / 22,888 MiB** — `scripts/k8s-env.sh`가 정본)으로 검증한
  K8s 재설계를, **비용 0**의 상시 클러스터(**2 OCPU/12 GB**)에서 재현한다. 포트폴리오상
  "IaC(Terraform)로 클라우드 K8s 프로비저닝" 경험을 더한다.
  > 한도 축소(2026-06) 이후 클라우드 쪽이 로컬(**8 CPU / 22,888 MiB**)보다 **작다**. "더 큰 클러스터"가 아니라
  > **상시 가동·IaC 경험**이 채택 이유로 남는다.
  > 🔴 로컬 상한이 6/16 → 8/22,888로 **올랐으므로 이 격차는 좁혀진 것이 아니라 벌어졌다** —
