...
## 이 프로젝트에서의 위치 — 🚧 채택·이행중(스트리밍 계층) · **오퍼레이터·Iceberg 연결 확인됨(2026-08-18)** · ⏸ **현재 미설치(2026-08-19)**

> **채택은 유지, 설치만 내렸다.** Phase 0 검증을 마친 세션 클러스터가 잡 없이 상주하며
> 1 CPU / 2Gi를 점유해 시분할 규약을 어기고 있어 오퍼레이터·`flink-session`·cert-manager를 제거했다.
> **"중단"과 "삭제"의 분리**(trino 선례와 같은 취지) — 자원은 즉시 회수하되 결정·검증 결과·
> 매니페스트·러너 이미지(`flink-runner:0.2.0`)는 남긴다. 복구는
> `INSTALL_FLINK=true ./scripts/k8s-operators.sh` 한 줄(롤백 비용 ≈ 0).
> 재개 시점은 [redesign.md](../redesign.md) **Phase 3**.
