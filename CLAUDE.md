...
- **에이전트 오케스트레이션·기록관**: AI 세션을 **3계층(supervisor→director→subagent)** 으로 나눈다(**director는 우선 1명**,
  ...
  **`security`·`archivist`·`skill-matcher`·`tech-writer` 4종은 director 관할 밖**(supervisor 직접 배정) —
  ...
  🔴 **`security` 최종 컨펌은 「계획(G1) 1회 + 작업내용(G2) 1회 + 계획 델타(Δ·조건부)」로 건다** —
  **G1·G2는 "한 벌"로** 올린다(쪼개면 **파일 사이의 조합에서 생기는 노출**을 구조적으로 못 본다 —
  쪼갠 판정의 합은 전체 판정이 아니다). **Δ 트리거**는 계획 밖의 ⓐ쓰기 경로 추가 ⓑ비가역 작업
  ⓒ외부 발신·반출이고, **비가역은 실행 *전에* 판정**해 G2로 미루지 않는다. Δ를 빼면 "계획만 통과시키고
  실행에서 이탈"이 무관측이 되므로 게이트를 좁힌 대가는 director의 **이탈 보고 의무**다.
  동일 결정 재컨펌 2회 초과 시 에스컬레이션. 🔴 **"호출이 줄었다"를 실효로 읽지 않는다**(원칙 7).
  미션 저널의 **기록 주체는 `archivist`** — supervisor가 체크포인트마다 이벤트를 전달해 기록시키고
  (경합 방지 single-writer), 호출 실패·세션 급종료·**워커 배정 불가** 시에만 supervisor가 **폴백**으로 쓴다. "누가 무엇을 왜
  ...
  **플랜 모드 계획서는 같은 볼트 `plans/<YYYY-MM-DD>/<NN>-<mission>.md`로 미러**된다
  (`scripts/plan_mirror_guard.py` — `PostToolUse`가 경로 수집, `Stop`이 소급 복사). 하네스가
  `~/.claude/plans/<랜덤슬러그>.md`로 **이름을 정해** 만들기 때문에 그대로 두면 어떤 미션의 계획인지
  알아볼 수 없다. 🔴 **미션 이름의 정본은 저널이라 미러는 이름을 짓지 않고 따라간다** — 저널이
  생길 때까지 보류했다 소급한다. 🔴 **저널과 계획서는 노출 통제의 축이 다르다** — 저널은 무엇을
  남길지 *고르며* 쓰지만 계획서는 하네스 산출물을 **통째로** 복사해 고르는 단계가 없다(병렬 세션이
  "취약점이 계획서 한 장에 집약돼 저널보다 밀도가 높다"고 제보). 볼트는 obsidian-git이 자동 푸시하므로
  **복사는 외부 발신**이고, 빼려면 계획서에 **`<!-- plan-mirror: off -->`** 를 둔다(기존 미러는 사람이 지운다).
  ...
  **스킬↔워커 배선은 계층 밖 `skill-matcher`(읽기 전용)** 가 감사한다 — `archivist`가 "저널 정합"을 보듯
  "어떤 스킬이 어떤 워커에 물렸나"를 본다. 등재 기준은 **게이트 2축 + 별점 3축**이다(2026-08-21 개정) —
  **게이트**(권한 정합·정본 무충돌)를 통과한 것만 **채점 3축**(스택 일치·호출 빈도·대체 불가)으로 매겨
  **★3(3축 전부)만 등재**한다. 게이트 탈락은 채점 없이 제외하되 **단서로 무해화 가능하면 통과**로 본다.
  🔴 **구 5축은 같은 축을 게이트이자 가점으로 이중 계상했다** — 축2·3에 거부권을 주면서 점수도 줘,
  축3·5(스킬측 속성)가 거의 전종 1이고 축2도 단서로 살아나 **기본값이 이미 ★3**이었다. 축1·축4 중
  **하나만** 1이면 ★4를 넘었으니 **5축이 실질 2축으로 작동**한 것이다(182셀 전수 실측으로 발견).
  🔴 **축을 재가중하면 구 판정이 그대로 유효하지 않다** — 축4 비중이 1/5→1/3으로 올랐으므로
  구 루브릭 판정을 재사용해 등재를 올리거나 내리지 않는다(재채점은 `skill-matcher` 소관).
  ...
  **분석은 새 축이 아니라 새 도메인**이라 3종을 복제하지 않고 **구현 축 `analyst` 1명**만 둔다(판정은 `data-*` 재사용).
  ...
  ✅ **①의 적용 대상은 「배선」(matcher·command)뿐이다** — **가드 스크립트 본문은 매 호출 즉시 반영**된다.
  즉 **로직 수정은 즉시, 프론트매터 수정만 새 세션**이다.
  🔴 **`director`·`researcher`의 가드 배선은 도구 경로에서 검증 불가**다 — `disallowedTools`가 `Write`를
  **먼저 제거**해 hook에 도달하지 않는다. 그 층은 `disallowedTools`가 뚫릴 때의 심층 방어다.
  🔴 프론트매터 `command`는 `settings.json`과 **인용 규칙이 다르다**(정본 `"$CLAUDE_PROJECT_DIR/scripts/….py"`) —
  틀리면 **에러 없이 조용히 통과**한다. 배선을 바꾸면 **실발동 확인을 다시 돌린다**.
  **같은 논리로 "공개"도 새 축이 아니라 새 도메인**이라 구현 축 **`tech-writer` 1명**만 둔다 —
  **저장소의 문서 소유자**로 `docs/**` 전체 + 최상위 `README.md`를 쓴다. 그 대가로
  ...
  (`CLAUDE.md`는 `docs/` 밖이라 가드가 **실제로** 막는다). 독자는 둘 — `docs/posts/**`는 **모르는 사람**,
  나머지는 **아는 사람**이 읽는다. 🔴 **매체는 축이 아니다**(지시문 **포맷 프로파일**로 흡수).
  🔴 **발행(업로드)은 어느 워커도 하지 않는다** — 외부 발신은 비가역이고 마지막 게이트는 **사람**이 갖는다.
  정본 [`docs/conventions/publishing.md`](docs/conventions/publishing.md) — **공개는 커밋보다 강한 기준**이다
  (내부 경로·버킷명·소규모 셀 <5·DUA 재배포 제한).
  **외부 근거는 도메인 공통 축 `researcher`** 가 맡는다(읽기 전용·`sonnet`) — **저장소의 유일한 외부
  ...
  🔴 **`ask`의 맨이름 `WebFetch`·`WebSearch`는 죽은 규칙이다** — 질의 유출을 사람이 볼 관측점이 없어
  이 규율의 실효는 **워커 자기 규율 100%**이고, 그 사실을 워커가 읽는 지시문에 적어야 작동한다.
  🔴 **`deny` 패턴은 명령 선두부터 앵커된다** — 플래그 순서만 바꿔도 통과한다(그래서 기존 규칙이
  앞뒤 `*`를 두른다). **새 `deny`/`ask` 패턴은 변형 2~3개로 반드시 재위반**한다. 문자열 매칭이라
  봉쇄가 아니라 **실수 방지**일 뿐 — 외부 발신의 진짜 방어선은 **사람 게이트**다.
  🔴 **워커 *신설*의 등록은 지연된다** — 만든 직후 호출이 `Agent type '...' not found`로 실패해도
  **같은 세션에서 몇 턴 뒤 등록된다**. 포기하지 말고 잠시 뒤 재시도한다(한 번의 부정 관측은
  *시점의 사실*이지 *구조의 사실*이 아니다 — 원칙 7).
  ...
  🔴 **워커에 물린 스킬은 ⚙️(디스크 설치)와 🌐(런타임 제공·디스크에 없음)를 가른다** — 워커는
  `Skill` 도구가 없어 ⚙️는 `Read`로 쓰지만 🌐는 **파일이 없어 `Read`도 불가**라 지시문에 적으면 죽은 참조다.
  ⚠️ **3계층 성립 여부는 `미확인`이다** — 서브에이전트의 `Agent` 도구 부재가 구조인지
  `disallowedTools` 파싱 실패의 부작용인지 갈리지 않았다. 열릴 때까지 **supervisor가 배정을 대행**하고
  `security` 컨펌도 supervisor가 수행한다. **새 세션에서 실호출로 재측정**한다(자기보고 아닌 런타임 응답으로).
  ...
  **`NotebookEdit`은 `notebook_path`** 라 한 가드만 뚫려 있었다. 가드가 여럿이라고
  **"하나가 막으니 다 막힌다"고 읽지 않는다**. 함정의 성립 조건은 둘 — ① matcher가 **여러 도구에 걸치고**
  ② 그 도구들 사이에서 **이름이 갈리는 필드**를 읽는다. 🔴 **내용 검사 가드를 새로 걸 때 재개된다**
  (내용 키도 `content`/`new_string`/`new_source`로 셋 다 다르다) — 그때는
  **matcher가 걸치는 도구 수 × 읽는 필드의 키 이름**을 먼저 표로 적는다.
  🔴 **hook 결정값은 `allow`·`deny`·`ask`·`defer` 넷뿐이다** — 값이 어긋나면 출력 전체가 거부되고
  결정이 폐기된 채 도구가 진행한다(**fail-open**). 🔴 **auto 모드에서 `ask`는 분류기가 흡수**하며,
  흡수 여부는 **도구 축**에 따라 갈린다 — `Bash`는 실제 위험 호출이면 발동하지만 **파일 도구
  (`Edit`/`Write`/`NotebookEdit`)는 경로 민감도와 무관하게 흡수**된다. ⇒ 급소는 "경로가 민감한가"가
  아니라 **"어느 도구인가"**이고, **파일 경로 경계는 `deny`여야 확실히 막힌다**. `ask`가 있다는 사실을
  "반드시 멈춘다"로 읽지 않는다(멈춤 = **규칙 × 분류기 판정**의 곱). 비가역 명령은 `ask` + 워커 지시문
  금지 조항의 **이중 방어**로 두고, 어느 쪽도 단독으로는 봉쇄가 아니다.
  🔴 **`ask`의 실효는 "일부러 위반"으로 확인할 수 없다**(원리상 — 프로브가 위험해야 뜨는데 위험하면
  못 돌린다). 임시 `deny` 전환으로만 **간접 확인**된다.
  🔴 **`permissions`(세션 도중 즉시)와 `hooks` 배선(정의 로드 시점 스냅샷)은 반영 시점이 반대다** —
  둘을 "설정 파일"로 묶어 같은 규칙으로 기억하면 틀린다(가드 스크립트 **본문**은 매 호출 즉시 반영).
  🔴 **선언 목록과 구현 목록은 주기적으로 대조한다** — 문서가 "`ask`로 못 박는다"고 적은 비가역 작업
  중 규칙이 아예 없던 것이 실제로 있었다. **가드가 막은 것과 분류기가 막은 것은 에러 문구 출처로
  구분**한다(`permissionDecisionReason` 원문 vs `denied by the Claude Code auto mode classifier`).
  실측 경위·6셀 대조·도구 축 분기표는 정본 [`docs/conventions/agents.md`](docs/conventions/agents.md) §hook 결정값.
  ...
  **피어 감지를 조용히 끈다**(레지스트리가 worktree마다 갈린다). 스크립트가 `.env`·`.claims`·
  `settings.local.json`을 **링크로 공유**해 이를 막는다. 🔴 **worktree는 파일·인덱스를 격리하지만
  클러스터·컨테이너는 격리하지 못한다** — 그래서 감지 축이 갈린다(git 축은 같은 트리로 한정,
  인프라·중복 축은 전체 유지). 🔴 **가드는 합성 페이로드로 테스트하면 실제 레지스트리를 바꾼다**
  (읽기 전용이 아니다 — 테스트 후 `.claude/.claims/sessions/<접두>.json`을 지운다).
  `.venv`는 editable 설치 때문에 **링크 금지**. **이미 시작한 세션은 이주 불가**라 효력은 다음 세션부터이고,
  공유 트리에서는 **pathspec 의무가 계속 유일한 방어선**이다.
  🔴 **피어에게 전달하는 저장소 상태는 관측 시점을 함께 적고, 집행 직전 `git log -1`로 재관측**한다 —
  관측과 집행 사이에 상대가 끼어든다. 피어 파일 지적은 **`git show HEAD:<path>`로 한 번 접어서** 보고,
  **같은 워킹트리인지 먼저 확인**한다(분리 전이면 남의 미커밋 변경이 `git status`에 보여
  `git add -A`가 딸려 간다 — **pathspec 필수**).
  🔴 **3자부터는 관측 *주체*도 함께 적는다** — `M` 표시도 mtime도 "누가"를 말해주지 않는다.
  `ListAgents`를 먼저 돌리고, **주체는 `session_id` 접두어**로 적는다(`ListAgents`의 ref는 관측자마다
  달라 커밋·저널처럼 **나중에 읽는 곳**에 남기면 틀린다).
  ...
  🔴 **저널과 `_MOC.md`는 한 벌로 갱신**한다(본문만 고치고 요약을 안 고치면 이중 스테일).
...
- **토큰 비용은 `요청 수 × 컨텍스트 크기`다**(2026-08-21 실측 — 캐시 읽기가 비용의 **62.5%**,
  ...
  🔴 **`CLAUDE.md`에 줄을 더하면 앞으로의 모든 요청에 곱해진다** — 회귀 실측상 기저의 **약 48%**가
  이 파일이다(0.521 토큰/바이트·R²=0.92, 세션 47개). 그래서 이 문서는 **규칙만** 두고
