...
> 데이터는 비식별 연구 데이터셋이지만 **DUA 대상**이다. 분석 산출물의 반출·공유 통제는
> [`../security.md`](../security.md)가 정본이고, 이 문서는 그 통제를 분석 작업 절차에 반영한다.
...
- **재식별을 시도하지 않는다.** 개별 환자 행을 산출물에 노출하지 않고, 집계 셀이 지나치게 작으면
  (관례적으로 5 미만) 마스킹하거나 구간을 넓힌다([`../security.md`](../security.md) 재식별 금지).

## 5. 리포트 규칙

- **위치는 `docs/analyses/<NN>-<slug>.md`** (첫 분석 때 디렉터리를 만든다).
