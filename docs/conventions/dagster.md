...
│   ├── resources.py        # 공유 Iceberg 카탈로그 설정 빌더 (catalog_properties · build_catalog_config)
...
3. **`defs/<dataset>/dbt_assets.py`** — `@dbt_assets(select="path:models/<dataset>")`로 dbt 모델 소유.
