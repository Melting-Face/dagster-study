...
| 6   | **추적 용이성**      | Locality of Behaviour (C. Gross) / *Readability counts.* (PEP 20) | wiring은 한 곳에 모아 중간 레이어를 줄이고(읽을 때 점프 최소화) · 값은 **named constant**로 선언해 grep/IDE 점프 가능 · 자산은 팩토리 대신 **명시 정의**로 이름 검색 · 로직 없는 단순 리턴 리소스는 빌더 없이 인라인 |
