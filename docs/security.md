...
| **3.4 파기 시 보호조치** | Iceberg **snapshot 만료 잡**(주간 스케줄, [defs/maintenance.py](../dagster/dockerfile.d/src/src/dagster_project/defs/maintenance.py)) | 🟡 | 보존기간(기본 7일)·대상 범위 확정, orphan 정리(Trino/Spark) 검토 필요([§4-1](#4-todo-실행-절차)) |
...
| ★★★★☆ | Iceberg snapshot **보존·파기** 자동화 — 🟡 잡 구현([defs/maintenance.py](../dagster/dockerfile.d/src/src/dagster_project/defs/maintenance.py)), 보존기간·orphan 정리 확정 잔여 | 3.4 · 2.9 | [operations.md §2](operations.md) |
...
대용량 테이블 `IcebergTableResource` 바인딩을 재사용한다. 실제 pyiceberg **0.11.x** API는
`table.maintenance.expire_snapshots().older_than(dt).commit()`(`older_than`는 tz-aware datetime).

```python
# defs/maintenance.py (요지)
SNAPSHOT_RETENTION_DAYS = 7  # 이보다 오래된 스냅샷 만료(현재/브랜치/태그는 자동 보호)


@dg.op
def expire_iceberg_snapshots(
    context: OpExecutionContext,
    mimiciv_chartevents_table: IcebergTableResource,  # 리소스 키로 주입
    ...
    for resource in (mimiciv_chartevents_table, mimiciv_labevents_table, eicu_nurse_charting_table):
        resource.load().maintenance.expire_snapshots().older_than(cutoff).commit()
```

> **orphan 파일 정리**: `remove_orphan_files`는 **pyiceberg 0.11.x 미지원**이라 잡에서 제외했다.
> 필요 시 Trino `ALTER TABLE ... EXECUTE remove_orphan_files` 또는 Spark 프로시저로 대체한다.
