...
- **`security`·`archivist`·`skill-matcher`·`tech-writer` 4종은 director 관할 밖**이다 —
  **supervisor가 직접 배정**한다(§워커 배치와 같은 목록. 근거도 거기 있다: 앞 3종은 **계층 자체를
  감사·기록**하기 때문이고, `tech-writer`는 **director의 행동 규칙이 담긴 정본을 쓰기** 때문이다).
- 🔴 **「관할 밖」과 「계층 밖」은 다른 축이다**(2026-08-20 피어 감사 지적 — 두 축이 한 라벨로 눌려 있었다).
  **관할 밖 = 4종**(위), **계층 밖 = `archivist`·`skill-matcher` 2종**(도메인 작업을 하지 않고
  **계층 자체를 감사·기록**한다). `security`는 **관할 밖이지만 계층 밖은 아니다** — 도메인 산출물을
  판정하는 워커이기 때문이다. 위 표의 `(계층 밖)` 라벨은 **`archivist`·`skill-matcher`에만** 붙는다.
