...
**판별은 대조로 했다**(원칙 7 — 관측이 가설을 분리하는가). "리스가 안 남는다"만으로는
*matcher 미발동*과 *경로 필터에 걸림* 두 가설이 갈리지 않는다. 그래서 **같은 디렉터리에**
`Write`를 한 번 더 쳤다:

| 셀 | 도구 | 대상 | 리스 등록 |
| --- | --- | --- | --- |
| 처리군 | `NotebookEdit` | `<scratch>/probe.ipynb` | **✕** |
| 대조군 | `Write` | `<scratch>/probe.txt` | **○** |

→ 경로가 원인이 아니므로 **키 불일치**로 확정. `PATH_KEYS = ("file_path", "notebook_path", "path")`로
고치고 **같은 프로브를 다시 쳐서** `tool = NotebookEdit` 리스가 남는 것을 확인했다(수정 전 ✕ → 수정 후 ○).
