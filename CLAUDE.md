...
- **Claude Code 스킬**: 프로젝트가 쓰는 Agent Skills와 사용 규칙(**프로젝트 컨벤션 우선**)은
  ...
  🔴 **lock 등재는 5/25다**(2026-08-21 00:53 실측 — 고유 종수 기준. 설치 *슬롯*으로는 26이고
  `sql-optimization`이 전역·프로젝트에 중복이다. **분모가 무엇을 세는지 함께 읽는다**).
  나머지 20종은 **설치돼 있으나 lock 밖**이고, "런타임이 제공하는 것"이 아니라
  **"설치했는데 lock에만 없는 것"** 이다. 🔴 **그러나 lock의 실효는 등재 수가 아니라 다른 데서 깨진다** —
  `computedHash`가 **무엇의 해시인지 몰라 로컬 재계산·대조가 불가능**하다(후보 3종 전부 불일치, `미확인`).
  즉 lock이 보장하는 것은 **출처·버전 기록까지**이고 무결성은 🔒에서도 검증되지 않는다.
